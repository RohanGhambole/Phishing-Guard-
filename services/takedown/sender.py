...
SENDER_PASS  = "xeux jwnj fiaa gdcn"   # 16-char app password, NOT your Gmail password
...
#     """Try RDAP first, then fall back to known registrars, then generic abuse@"""
#     # strip port if present
#     clean_domain = domain.split(':')[0]

#     try:
#         async with httpx.AsyncClient(timeout=8) as c:
#             r = await c.get(f'https://rdap.org/domain/{clean_domain}')
...
#                             if field[0] == 'email':
#                                 email = field[3]
#                                 print(f"  Found abuse contact via RDAP: {email}")
#                                 return email
#     except Exception:
#         pass
